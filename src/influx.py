...
def get_thingy_characteristic(thingy, characteristic):
    rs = client.query(
        'SELECT * FROM "' + characteristic + '" WHERE "thingy"=\'' + thingy + "'"
    )
    return list(rs.get_points())


def get_characteristic_by_date_range(characteristic, start_date, end_date, thingy):
    query = (
        'SELECT * FROM "'
        + characteristic
        + '" WHERE "thingy"=\''
...
def get_thingy_last_characteristic(thingy, characteristic):
    rs = client.query(
        'SELECT * FROM "' + characteristic + '" GROUP BY * ORDER BY DESC LIMIT 1'
    )
...
def create_retention_policy(name, duration, default):
    client.create_retention_policy(
        name, duration, 1, database="purple", default=default, shard_duration=duration
    )
